						...
						<div class="col-8"  style="margin-left: 75%;">
							<table class="table table-bordered text-center" style="border: 2px solid #151414;">
								...
								<tr style="border: 2px solid #151414;">
										...
										if($no_of_rows>0):
											$Each_row_value=0;
												while($Each_row=mysqli_fetch_array($query_statement)):
													...
													$seen_query = "SELECT count(seen) as unseen FROM user_chat where (from_user='$from_user' and to_user='$to_user' and seen='0') or (from_user='$to_user' and to_user='$from_user' and seen='0')";
									...
									<td class="mt-3 mb-4"> 
										<a href='index.php?customer_id=<?= $Each_row["customer_id"]?>' class="btn btn-success" title="<?= $seen_data_row["unseen"]?> unread message" >
											<i class="far fa-comment" style="font-size:24px;border-radius:15px; position:relative;"><?= $seen_data_row["unseen"]?></i>

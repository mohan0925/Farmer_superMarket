...
{
    $vendor_id = $_GET['enable'];
    $enable_query=mysqli_query( $connecti,"UPDATE product_vendor set vendor_status='Active' WHERE vendor_id='$vendor_id'");
    ...
    if(is_bool($enable_query)=== true)
	{
		...
		if($count>0)
		{
			...
			$enable_query=mysqli_query( $connecti,"UPDATE products set product_status='Available' WHERE vendor_email='$vendor_email'");
